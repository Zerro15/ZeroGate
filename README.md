...
## Архитектура
Проект разделён на два основных блока: backend (FastAPI) и клиент (Flutter). Общение идёт через REST API. В качестве хранилища используется SQLite (по умолчанию), а переход на PostgreSQL возможен благодаря SQLAlchemy.
